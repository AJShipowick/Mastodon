namespace OsOEasy_API.Responses
{
    ...
    public class MainJS : IMainJS
    {
        ...
        public string GetMainJS(string promotionId)
        {
            ...
            string minJS = "(function(){let sliderDiv=document.createElement('div');sliderDiv.setAttribute('id','osoContainer');document.body.appendChild(sliderDiv);loadSlickHTML();loadSlickCSS();loadSlickJS();setTimeout(function(){loadSlickImage()},500);function loadSlickHTML(){let htmlURL='http://localhost:51186/api/promo/html/?';getSlickResource(htmlURL,handleHTMLCallback)}function handleHTMLCallback(data){document.getElementById('osoContainer').innerHTML=data}function loadSlickCSS(){let cssURL='http://localhost:51186/api/promo/css/?';getSlickResource(cssURL,handleCSSCallback)}function handleCSSCallback(data){let style=document.createElement('style');style.type='text/css';style.innerHTML=data;document.getElementsByTagName('head')[0].appendChild(style)}function loadSlickJS(){let jsURL='http://localhost:51186/api/promo/js/?';getSlickResource(jsURL,handleJSCallback)}function handleJSCallback(data){let script=document.createElement('script');script.innerHTML=data;document.getElementsByTagName('head')[0].appendChild(script)}function loadSlickImage(){let imageURL='http://localhost:51186/api/promo/image/?';getImageResource(imageURL,handleImageCallback)}function handleImageCallback(data){showSlickImageOnScreen(data)}function getImageResource(imageURL,callback){let oReq=new XMLHttpRequest();oReq.open('GET',imageURL,true);oReq.responseType='blob';oReq.onload=function(oEvent){callback(oReq.response)};oReq.send()}function showSlickImageOnScreen(blobData){let urlCreator=window.URL||window.webkitURL;let imageUrl=urlCreator.createObjectURL(blobData);document.querySelector('#osoImage').src=imageUrl}})();function submitSlider(){let name=document.getElementById('osoUserName').value;let email=document.getElementById('osoUserEmail').value;if(!name||!email){let responseMsg=document.getElementById('osoPromoResponseMessage');responseMsg.innerHTML='Please fill out all form fields.';responseMsg.style.color='red';return}let submitURL='http://localhost:51186/api/promo/submit/?/'+name+'/'+email;getSlickResource(submitURL,handleSubmitCallback)}function getSlickResource(resourceURL,callback){let xhr=new XMLHttpRequest();xhr.onreadystatechange=function(){if(xhr.readyState===4){callback(xhr.response);}else{console.log('error getting resource: '+resourceURL)}};xhr.open('GET',resourceURL,true);xhr.send()}function handleSubmitCallback(data){console.log(data);}";
